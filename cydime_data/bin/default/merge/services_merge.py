...
def merge_features(infilename, outfilename, dest_dir, fname):
    ...
    with open(infilename, "r") as infile:
        ...
        for line in infile:
            ...
            ip_dict[key].tot_rec += int(line[3])
            ip_dict[key].tot_pak += int(line[4])
            ip_dict[key].tot_byt += int(line[5])
    ...
    with open(output_file, "w") as output:
        output.write("IP,INproto/service,OUTproto/service,Records,Packets,Bytes\n")
        for ip in d:
